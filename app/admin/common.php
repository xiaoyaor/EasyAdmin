...
if (!function_exists('build_heading')) {
    ...
    function build_heading($path = NULL, $container = TRUE)
    {
        ...
        if (is_null($path)) {
            ...
            $path = $addonsName?$addonsName.'/':null;
            $path = strtolower($path.$controller . ($action && $action != 'index' ? '/' . $action : ''));
        }
        ...
        if($isTable){
            ...
            $data = !$data?Db::name('auth_rule')->where('name', $addonsName)->field('title,remark')->find():$data;
        }else{
