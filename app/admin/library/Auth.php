...
class Auth extends ThinkAuth
{
    ...
    public function login($username, $password, $keeptime = 0)
    {
        ...
        if (Config::get('easyadmin.login_failure_retry') && $admin->loginfailure >= 10 && time() - $admin->updatetime < 86400) {
    ...
    public function logout()
    {
        ...
        $admin = $adminmodel->find(intval($this->id));
        if (!$admin) {
            $admin->token = '';
            $admin->save();
        }
